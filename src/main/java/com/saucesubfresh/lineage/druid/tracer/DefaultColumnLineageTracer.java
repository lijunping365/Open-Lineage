...
public class DefaultColumnLineageTracer implements ColumnLineageTracer {
  ...
  /** Long tableId 节点ID List<TreeNode<TableNode>>> 某一节点最近的节点 */
   ...
   */
  @Override
  public void traceColumnLineageTree(
    ...
    for (TreeNode<TableNode> currentRecentlyTableNode : nearestTableNodeList) {
      ...
      String scanColumnName =
          Optional.ofNullable(currentColumnNode.getValue().getName())
  ...
  private void possibleColumnSource(
      TreeNode<ColumnNode> currentColumnNode, List<TreeNode<TableNode>> nearestTableNodeList) {
    if (!CollectionUtils.isEmpty(nearestTableNodeList)
        && nearestTableNodeList.size() == 1
   ...
   */
  private void nearestTableNodes(
      TreeNode<TableNode> currentNode, List<TreeNode<TableNode>> nearestTableNodeList) {
    ...
    currentNode
        ...
        .forEach(
            node -> {
              // 子节点，找到就结束
              if (node.getValue().getAlias() != null
                  || node.getValue().getIsVirtualTemp() == null) {
  ...
  /**
   * 修复缺失的表名 for example: 1. select distinct a1,b1 from table1 V 2. select distinct a1,b1 from
   * table1,table2 X 第一种情况识别可以，第二种会取第一个表的字段，建议在SQL书写时，指定出对应别名
   *
  ...
  private String repairMissingTableName(ColumnNode columnNode, String dbType) {
    ...
    return pgSchemaStatVisitor.getTables().keySet().stream()
        .findFirst()
        .orElseThrow(
            () ->
                new ParserException(
                    "repair missing table failed,column expression[%s].",
